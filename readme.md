...
Tested only on the LilyGo T-Dongle.

Usage:
* **Insert USB**. *Access point*: Basic file management usage.
* **Click**. *Swiss Army functionality*: Enable full usage of device through the AP. Note you have to click to this point to get full access (the wait_for_button command otherwise prevents a lot of functionality).
...
* **Long press**. *USB ethernet capture*: On click it perform a setup, reboot, and displays a graph as it gathers an Ethernet pcap; afterwards it reboots back into normal mode.
* **Long press**. *USB storage*: Press to activate. 

The excellent covert USB example from the original is included, but requires manual intervention. It now has a new disguise.
1) Backup autorun.ds
2) Copy coverstorage_autorrun.ds and paste as new autorun.ds
3) Next time you plug in USB, it will show as a storage device but only once. 
